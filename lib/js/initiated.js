...
window.onload = function () {
  var ls = new URLSearchParams(location.search);

  if (ls.get('mode') == 1) {
    var message = encodeURIComponent(btoa(ls.get('message')));

    return location.assign(`${location.pathname}?mode=0&message=${message}`);
  }

  var message = decodeURIComponent(atob(ls.get('message')));

  document.getElementById('recipient').innerHTML = message;
